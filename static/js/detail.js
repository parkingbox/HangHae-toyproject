...
function show_coment(param) {
  $("#coment-list").empty();
  $.ajax({
    type: "POST",
    url: "/comentshow",
    data: { give_contentid: param },
    success: function (response) {
      let rows = response["comentlist"];
      for (let i = 0; i < rows.length; i++) {
        let coment = rows[i]["coment"];
        let num = rows[i]["num"];
        let contentid = rows[i]["contentid"];

        let temp_html = `<li>
          <h2>✅${coment}</h2>
          <button onclick="delete_coment(${num})" type="button" class="btn btn-outline-primary">삭재!</button>
        </li>`;
